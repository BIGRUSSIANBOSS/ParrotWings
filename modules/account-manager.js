...
var dbName = process.env.DB_NAME || 'pw';
...
exports.createTransaction = function(newData, callback)
{
    transactions.insertOne(newData, {safe: true}, callback);
};
...
{
    accounts.findOne({user: user}, function(e, o){
        transactions.find({$or: [{source: user},{target: user}]}).toArray(function(err, results){
            console.log(o);
            var account = o;
            account.transactions = results;
            callback(account); // output all records
        });
    });
}
